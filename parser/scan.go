...
func (s *scanner) scanNumber(lval *sqlSymType, ch int) {
	...
	if hasDecimal || hasExponent {
		...
		lval.union.val = ast.NewNumVal(floatConst, lval.str, false /* negative */)
	} else {
		...
		lval.union.val = ast.NewNumVal(intConst, lval.str, false /* negative */)
	}
}
...
func (s *scanner) scanPlaceholder(lval *sqlSymType) {
	...
	placeholder, err := ast.NewPlaceholder(lval.str)
	if err != nil {
		lval.id = ERROR
		lval.str = err.Error()
		return
