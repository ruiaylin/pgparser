...
func short(in string) string {
	return strings.Replace(in, "github.com/ruiaylin/pgparser/pkg/", "./pkg/", -1)
}
	...
	check = func(path string) error {
		...
		for _, imp := range pkg.Imports {
			for _, forbidden := range forbiddenPkgs {
				...
				if forbidden == "c-deps" &&
					imp == "C" &&
					strings.HasPrefix(path, "github.com/ruiaylin/pgparser/pkg") &&
					path != "github.com/ruiaylin/pgparser/types/geo/geoproj" {
...
func VerifyTransitiveAllowlist(t testing.TB, pkg string, allowedPkgs []string) {
	...
	check = func(path string) {
		...
		checked[path] = struct{}{}
		if strings.HasPrefix(path, "github.com/ruiaylin/pgparser/vendor") {
			return
		}
		...
		for _, imp := range pkg.Imports {
			if !strings.HasPrefix(imp, "github.com/ruiaylin/pgparser/") {
				continue
			}
