...
func genValues(w io.Writer, firstRow, lastRow int, fn GenRowFn, shouldPrint bool) {
	for rowIdx := firstRow; rowIdx <= lastRow; rowIdx++ {
		...
		fmt.Fprintf(w, "(%s", ast.Serialize(row[0]))
		for _, v := range row[1:] {
			fmt.Fprintf(w, ",%s", ast.Serialize(v))
		}
		fmt.Fprint(w, ")")
	}
}
...
func RowModuloShiftedFn(modulo ...int) GenValueFn {
	return func(row int) ast.Datum {
		return ast.NewDInt(ast.DInt(moduloShiftedRecursive(row, modulo)))
	}
}
...
func RowEnglishFn(row int) ast.Datum {
	return ast.NewDString(IntToEnglish(row))
}
