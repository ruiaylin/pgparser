...
type Statement struct {
	...
	// NumAnnotations indicates the number of annotations in the ast. It is equal
	// to the maximum annotation index.
	NumAnnotations ast.AnnotationIdx
}
...
func (stmts Statements) StringWithFlags(flags ast.FmtFlags) string {
	ctx := ast.NewFmtCtx(flags)
	for i, s := range stmts {
		if i > 0 {
			ctx.WriteString("; ")
		}
...
func ParseQualifiedTableName(sql string) (*ast.TableName, error) {
...
func ParseTableName(sql string) (*ast.UnresolvedObjectName, error) {
	...
	rename, ok := stmt.AST.(*ast.RenameTable)
	if !ok {
...
func ParseType(sql string) (ast.ResolvableTypeReference, error) {
	...
	if !ok {
		return nil, errors.AssertionFailedf("expected a ast.CastExpr, but found %T", expr)
	}
