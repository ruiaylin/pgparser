...
import (
	...
	"github.com/ruiaylin/pgparser/pkg/base"
	"github.com/ruiaylin/pgparser/pkg/col/coldata"
	"github.com/ruiaylin/pgparser/settings/cluster"
	"github.com/ruiaylin/pgparser/colexec"
	"github.com/ruiaylin/pgparser/colexec/colbuilder"
	"github.com/ruiaylin/pgparser/colexecbase"
	"github.com/ruiaylin/pgparser/execinfra"
	"github.com/ruiaylin/pgparser/execinfrapb"
	"github.com/ruiaylin/pgparser/parser"
	"github.com/ruiaylin/pgparser/rowexec"
	...
	t.Run("sql", func(t *testing.T) {
		...
		walk(t, func(t *testing.T, d *datadriven.TestData) string {
			...
			case types.TupleFamily:
				...
				var datums ast.Datums
				// Fetch the original expression's tuple values.
				tuple := typedExpr.(*ast.Tuple)
				for i, s := range strings.Split(res.String[1:len(res.String)-1], ",") {
					...
					datum, _, err := ast.ParseAndRequireString(expr.ResolvedType(), s, evalCtx)
					if err != nil {
						t.Errorf("%s: %s", err, s)
						return err.Error()
					}
				...
				return ast.NewDTuple(typedExpr.ResolvedType(), datums...).String()
			}
			datum, _, err := ast.ParseAndRequireString(typedExpr.ResolvedType(), res.String, evalCtx)
			if err != nil {
	...
	t.Run("vectorized", func(t *testing.T) {
		walk(t, func(t *testing.T, d *datadriven.TestData) string {
			if d.Input == "B'11111111111111111111111110000101'::int4" {
				// Skip this test: https://github.com/ruiaylin/pgparser/pull/40790#issuecomment-532597294.
