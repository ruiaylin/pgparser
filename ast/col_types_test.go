...
func TestParseColumnType(t *testing.T) {
	...
	for i, d := range testData {
		t.Run(d.str, func(t *testing.T) {
			...
			createTable, ok := stmt.AST.(*ast.CreateTable)
			if !ok {
				t.Fatalf("%d: expected ast.CreateTable, but got %T", i, stmt)
			}
			columnDef, ok2 := createTable.Defs[0].(*ast.ColumnTableDef)
			if !ok2 {
				t.Fatalf("%d: expected ast.ColumnTableDef, but got %T", i, createTable.Defs[0])
			}
			colType := ast.MustBeStaticallyKnownType(columnDef.Type)
...
func TestParseColumnTypeAliases(t *testing.T) {
	...
	for i, d := range testData {
		t.Run(d.str, func(t *testing.T) {
			...
			createTable, ok := stmt.AST.(*ast.CreateTable)
			if !ok {
				t.Fatalf("%d: expected ast.CreateTable, but got %T", i, stmt.AST)
			}
			columnDef, ok2 := createTable.Defs[0].(*ast.ColumnTableDef)
			if !ok2 {
				t.Fatalf("%d: expected ast.ColumnTableDef, but got %T", i, createTable.Defs[0])
