...
func VerifyStatementPrettyRoundtrip(t *testing.T, sql string) {
	...
	for i := range stmts {
		...
		prettyFormatted := ast.AsStringWithFlags(parsedPretty.AST, ast.FmtSimple)
		origFormatted := ast.AsStringWithFlags(origStmt, ast.FmtParsable)
		if prettyFormatted != origFormatted {
			...
			origFormatted = ast.AsStringWithFlags(reparsedStmt.AST, ast.FmtParsable)
			if prettyFormatted != origFormatted {
