...
import (
	...
	"github.com/ruiaylin/pgparser/sqlutils"
...
func TestClassifyTablePattern(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.in, func(t *testing.T) {
			tp, err := func() (ast.TablePattern, error) {
				stmt, err := parser.ParseOne(fmt.Sprintf("GRANT SELECT ON %s TO foo", tc.in))
				if err != nil {
					return nil, err
				}
				tp, err := stmt.AST.(*ast.Grant).Targets.Tables[0].NormalizeTablePattern()
				if err != nil {
					return nil, err
				}
				return tp, nil
...
func TestClassifyColumnName(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.in, func(t *testing.T) {
			v, err := func() (ast.VarName, error) {
				stmt, err := parser.ParseOne(fmt.Sprintf("SELECT %s", tc.in))
				if err != nil {
					return nil, err
				}
				v := stmt.AST.(*ast.Select).Select.(*ast.SelectClause).Exprs[0].Expr.(ast.VarName)
...
type knownTable struct {
	srcName ast.TableName
	columns []ast.Name
}

type colsRes ast.NameList
...
func (f *fakeSource) FindSourceMatchingName(
	_ context.Context, tn ast.TableName,
) (
	res ast.NumResolutionResults,
	prefix *ast.TableName,
	srcMeta ast.ColumnSourceMeta,
	err error,
) {
	defer func() {
	...
	for i := range f.knownTables {
		...
		if found {
			return ast.MoreThanOne, nil, nil, fmt.Errorf("ambiguous source name: %q", &tn)
		}
		found = true
		prefix = &t.srcName
	...
	if !found {
		return ast.NoResults, nil, nil, nil
	}
	return ast.ExactlyOne, prefix, columns, nil
}

// FindSourceProvidingColumn is part of the ColumnItemResolver interface.
func (f *fakeSource) FindSourceProvidingColumn(
	_ context.Context, col ast.Name,
) (prefix *ast.TableName, srcMeta ast.ColumnSourceMeta, colHint int, err error) {
	defer func() {
...
func (f *fakeSource) ambiguousColumnErr(col ast.Name) error {
	...
	for i := range f.knownTables {
		...
		for _, cn := range t.columns {
			if cn == col {
				fmt.Fprintf(&candidates, "%s%s.%s", sep, ast.ErrString(&t.srcName), cn)
				sep = ", "
			}
		}
	}
...
func (f *fakeSource) Resolve(
	...
	colHint int,
	col ast.Name,
) (ast.ColumnResolutionResult, error) {
...
// interface.
func (f *fakeSource) GetColumnItemResolver() ast.ColumnItemResolver {
	return f
}

// AddTable is part of the sqlutils.ColumnItemResolverTester interface.
func (f *fakeSource) AddTable(tabName ast.TableName, colNames []ast.Name) {
...
// sqlutils.ColumnItemResolverTester interface.
func (f *fakeSource) ResolveColumnItemTestResults(res ast.ColumnResolutionResult) (string, error) {
	c, ok := res.(colRes)
	if !ok {
...
) (found bool, scMeta ast.SchemaMeta, err error) {
	defer func() {
...
func (f *fakeMetadata) LookupObject(
	_ context.Context, lookupFlags ast.ObjectLookupFlags, dbName, scName, tbName string,
) (found bool, obMeta ast.NameResolutionResult, err error) {
	defer func() {
...
func newFakeMetadata() *fakeMetadata {
	return &fakeMetadata{
		knownVSchemas: []knownSchema{
			{"pg_catalog", []ast.Name{"pg_tables"}},
		},
		knownCatalogs: []knownCatalog{
			{"db1", []knownSchema{{"public", []ast.Name{"foo", "kv"}}}},
			{"db2", []knownSchema{
				{"public", []ast.Name{"foo"}},
				{"extended", []ast.Name{"bar", "pg_tables"}},
			}},
			{"db3", []knownSchema{
				{"public", []ast.Name{"foo", "bar"}},
				{"pg_temp_123", []ast.Name{"foo", "baz"}},
			}},
		},
	}
}
...
func TestResolveTablePatternOrName(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("%s/%s/%s/%v", tc.in, tc.curDb, tc.searchPath, tc.expected), func(t *testing.T) {
			fakeResolver.t = t
			tp, sc, err := func() (ast.TablePattern, string, error) {
				...
				case *ast.TableName:
					var prefix ast.ObjectNamePrefix
					if tc.expected {
						flags := ast.ObjectLookupFlags{}
						...
						found, prefix, obMeta, err = ast.ResolveExisting(ctx, un, fakeResolver, flags, tc.curDb, tc.searchPath)
					} else {
						// TODO: As part of work for #34240, we should be operating on
						//  UnresolvedObjectNames here, rather than TableNames.
						un := tpv.ToUnresolvedObjectName()
						found, prefix, scMeta, err = ast.ResolveTarget(ctx, un, fakeResolver, tc.curDb, tc.searchPath)
					}
