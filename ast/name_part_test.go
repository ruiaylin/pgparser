...
func TestUnresolvedNameAnnotation(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	aIdx := ast.AnnotationIdx(1)
	u, err := ast.NewUnresolvedObjectName(1, [3]string{"t"}, aIdx)
	if err != nil {
		t.Fatal(err)
	}
	ann := ast.MakeAnnotations(aIdx)

	expect := func(expected string) {
		t.Helper()
		ctx := ast.NewFmtCtxEx(ast.FmtAlwaysQualifyTableNames, &ann)
		ctx.FormatNode(u)
