...
import (
	...
	"github.com/ruiaylin/pgparser/opt/exec/execbuilder"
	"github.com/ruiaylin/pgparser/opt/optbuilder"
	"github.com/ruiaylin/pgparser/opt/xform"
...
func TestEval(t *testing.T) {
	...
	walkExpr := func(t *testing.T, getExpr func(ast.Expr) (ast.TypedExpr, error)) {
	...
	t.Run("opt", func(t *testing.T) {
		walkExpr(t, func(e ast.Expr) (ast.TypedExpr, error) {
...
func optBuildScalar(evalCtx *ast.EvalContext, e ast.Expr) (ast.TypedExpr, error) {
	...
	ivh := ast.MakeIndexedVarHelper(nil /* container */, 0)
...
func TestEvalError(t *testing.T) {
	...
	for _, d := range testData {
		...
		semaCtx := ast.MakeSemaContext()
		typedExpr, err := ast.TypeCheck(ctx, expr, &semaCtx, types.Any)
		if err == nil {
			evalCtx := ast.NewTestingEvalContext(cluster.MakeTestingClusterSettings())
