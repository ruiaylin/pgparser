...
func BenchmarkTypeCheck(b *testing.B) {
	...
	ctx := ast.MakeSemaContext()
	if err := ctx.Placeholders.Init(1 /* numPlaceholders */, nil /* typeHints */); err != nil {
		b.Fatal(err)
	}
	for i := 0; i < b.N; i++ {
		_, err := ast.TypeCheck(context.Background(), expr, &ctx, types.Int)
		if err != nil {
...
func TestTypeCheckNormalize(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.expr, func(t *testing.T) {
			...
			if s := ast.Serialize(typedExpr); s != d.expected {
...
var (
	ptypesNone              = ast.PlaceholderTypes{nil, nil}
	ptypesInt               = ast.PlaceholderTypes{types.Int, nil}
	ptypesDecimal           = ast.PlaceholderTypes{types.Decimal, nil}
	ptypesIntAndInt         = ast.PlaceholderTypes{types.Int, types.Int}
	ptypesIntAndDecimal     = ast.PlaceholderTypes{types.Int, types.Decimal}
	ptypesDecimalAndDecimal = ast.PlaceholderTypes{types.Decimal, types.Decimal}
)

// copyableExpr can provide each test permutation with a deep copy of the expression ast.
type copyableExpr func() ast.Expr

func buildExprs(exprs []copyableExpr) []ast.Expr {
	freshExprs := make([]ast.Expr, 0, len(exprs))
...
func intConst(s string) copyableExpr {
	return func() ast.Expr {
		return ast.NewNumVal(constant.MakeFromLiteral(s, token.INT, 0), s, false /* negative */)
	}
}
func decConst(s string) copyableExpr {
	return func() ast.Expr {
		return ast.NewNumVal(constant.MakeFromLiteral(s, token.FLOAT, 0), s, false /* negative */)
	}
}
func dint(i ast.DInt) copyableExpr {
	return func() ast.Expr {
		return ast.NewDInt(i)
	}
}
...
func placeholder(id ast.PlaceholderIdx) copyableExpr {
	return func() ast.Expr {
...
func tuple(exprs ...copyableExpr) copyableExpr {
	return func() ast.Expr {
		return &ast.Tuple{Exprs: buildExprs(exprs)}
	}
}
...
func clonePlaceholderTypes(args ast.PlaceholderTypes) ast.PlaceholderTypes {
	clone := make(ast.PlaceholderTypes, len(args))
	for k, v := range args {
		clone[k] = v
	}
	return clone
...
func attemptTypeCheckSameTypedExprs(t *testing.T, idx int, test sameTypedExprsTestCase) {
	...
	forEachPerm(test.exprs, 0, func(exprs []copyableExpr) {
		...
		_, typ, err := ast.TypeCheckSameTypedExprs(ctx, &semaCtx, desired, buildExprs(exprs)...)
		if err != nil {
...
func TestTypeCheckSameTypedExprsError(t *testing.T) {
	...
	for i, d := range testData {
		...
		forEachPerm(d.exprs, 0, func(exprs []copyableExpr) {
			if _, _, err := ast.TypeCheckSameTypedExprs(ctx, &semaCtx, desired, buildExprs(exprs)...); !testutils.IsError(err, d.expectedErr) {
...
func cast(p *ast.Placeholder, typ *types.T) ast.Expr {
	return &ast.CastExpr{Expr: p, Type: typ}
}
func annot(p *ast.Placeholder, typ *types.T) ast.Expr {
	return &ast.AnnotateTypeExpr{Expr: p, Type: typ}
}
...
func TestProcessPlaceholderAnnotations(t *testing.T) {
	...
	testData := []struct {
		initArgs  ast.PlaceholderTypes
		stmtExprs []ast.Expr
		desired   ast.PlaceholderTypes
	}{
		{ // 0
			ast.PlaceholderTypes{nil},
			[]ast.Expr{newPlaceholder(0)},
			ast.PlaceholderTypes{nil},
		},
		{ // 1
			ast.PlaceholderTypes{nil, nil},
			[]ast.Expr{newPlaceholder(0), newPlaceholder(1)},
			ast.PlaceholderTypes{nil, nil},
		},
		{ // 2
			ast.PlaceholderTypes{nil, nil, types.Bool},
			[]ast.Expr{newPlaceholder(0), newPlaceholder(1)},
			ast.PlaceholderTypes{nil, nil, types.Bool},
		},
		{ // 3
			ast.PlaceholderTypes{nil, nil, types.Float},
			[]ast.Expr{newPlaceholder(0), newPlaceholder(1)},
			ast.PlaceholderTypes{nil, nil, types.Float},
		},
		{ // 4
		...
		{ // 5
			ast.PlaceholderTypes{types.Float},
			[]ast.Expr{
				cast(newPlaceholder(0), intType),
				cast(newPlaceholder(0), boolType),
			},
		...
		{ // 6
			...
			ast.PlaceholderTypes{types.Int, types.Bool},
		},
		{ // 7
			ast.PlaceholderTypes{nil, nil},
			[]ast.Expr{
		...
		{ // 8
			ast.PlaceholderTypes{nil, types.Bool},
			[]ast.Expr{
				annot(newPlaceholder(0), intType),
			},
			ast.PlaceholderTypes{types.Int, types.Bool},
		...
		{ // 9
			...
			ast.PlaceholderTypes{types.Int, nil},
		},
		{ // 10
			ast.PlaceholderTypes{nil, nil},
			[]ast.Expr{
		...
		{ // 15
			ast.PlaceholderTypes{types.Float, types.Bool},
			[]ast.Expr{
				newPlaceholder(0),
				cast(newPlaceholder(0), intType),
				cast(newPlaceholder(1), boolType),
		...
		{ // 16
			ast.PlaceholderTypes{types.Float, types.Float},
			[]ast.Expr{
				newPlaceholder(0),
				cast(newPlaceholder(0), intType),
				cast(newPlaceholder(1), boolType),
		...
		{ // 17
			...
			ast.PlaceholderTypes{types.Int},
		},
		{ // 18
			ast.PlaceholderTypes{nil},
			[]ast.Expr{
			...
			ast.PlaceholderTypes{types.Bool},
		},
	}
	for i, d := range testData {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			args := d.initArgs
			stmt := &ast.ValuesClause{Rows: []ast.Exprs{d.stmtExprs}}
			if err := ast.ProcessPlaceholderAnnotations(&semaCtx, stmt, args); err != nil {
...
func TestProcessPlaceholderAnnotationsError(t *testing.T) {
	...
	for i, d := range testData {
		args := d.initArgs
		stmt := &ast.ValuesClause{Rows: []ast.Exprs{d.stmtExprs}}
		if err := ast.ProcessPlaceholderAnnotations(&semaCtx, stmt, args); !testutils.IsError(err, d.expected) {
			t.Errorf("%d: expected '%s', got '%v'", i, d.expected, err)
		}
	}
}

func newPlaceholder(id ast.PlaceholderIdx) *ast.Placeholder {
	return &ast.Placeholder{Idx: id}
}
