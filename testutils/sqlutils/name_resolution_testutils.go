...
// struct that also implements ast.ColumnItemResolver. It is used to test that
// the implementation of ast.ColumnItemResolver is correct.
...
func initColumnItemResolverTester(t *testing.T, ct ColumnItemResolverTester) {
	ct.AddTable(ast.MakeTableNameWithSchema("", "crdb_internal", "tables"), []ast.Name{"table_name"})
	ct.AddTable(ast.MakeTableName("db1", "foo"), []ast.Name{"x"})
	ct.AddTable(ast.MakeTableName("db2", "foo"), []ast.Name{"x"})
	ct.AddTable(ast.MakeUnqualifiedTableName("bar"), []ast.Name{"x"})
	ct.AddTable(ast.MakeTableName("db1", "kv"), []ast.Name{"k", "v"})
}
	...
	for _, tc := range testCases {
		t.Run(tc.in, func(t *testing.T) {
			tnout, csout, err := func() (string, string, error) {
				...
				acs, ok := c.(*ast.AllColumnsSelector)
				if !ok {
