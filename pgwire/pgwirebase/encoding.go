...
import (
	...
	"github.com/ruiaylin/pgparser/oidext"
	...
	"github.com/ruiaylin/pgparser/utils/bitarray"
	...
	"github.com/ruiaylin/pgparser/utils/timeutil/pgdate"
	"github.com/ruiaylin/pgparser/utils/uint128"
...
// will be applied.
func DecodeOidDatum(
	ctx ast.ParseTimeContext, id oid.Oid, code FormatCode, b []byte,
) (ast.Datum, error) {
	switch code {
	case FormatText:
		switch id {
		case oid.T_bool:
			...
			return ast.MakeDBool(ast.DBool(t)), nil
		case oid.T_bit, oid.T_varbit:
			t, err := ast.ParseDBitArray(string(b))
			if err != nil {
				return nil, err
			}
			return t, nil
		...
		case oid.T__int2, oid.T__int4, oid.T__int8:
			...
			out := ast.NewDArray(types.Int)
			var d ast.Datum
			for _, v := range arr.Elements {
				if v.Status != pgtype.Present {
					d = ast.DNull
				} else {
					d = ast.NewDInt(ast.DInt(v.Int))
				}
		...
		case oid.T__text, oid.T__name:
			...
			out := ast.NewDArray(types.String)
			if id == oid.T__name {
			...
			for _, v := range arr.Elements {
				if v.Status != pgtype.Present {
					...
					d = ast.NewDString(v.String)
					if id == oid.T__name {
						d = ast.NewDNameFromDString(d.(*ast.DString))
					}
				}
				if err := out.Append(d); err != nil {
					return nil, err
		...
		case oid.T_jsonb:
			...
			return ast.ParseDJSON(string(b))
		}
		if _, ok := types.ArrayOids[id]; ok {
			...
			return ast.NewDString(string(b)), nil
		}
	case FormatBinary:
		switch id {
		case oid.T_bool:
			if len(b) > 0 {
				switch b[0] {
				case 0:
					return ast.MakeDBool(false), nil
				case 1:
					return ast.MakeDBool(true), nil
				}
			}
		...
		case oid.T_time:
			...
			i := int64(binary.BigEndian.Uint64(b))
			return ast.MakeDTime(timeofday.TimeOfDay(i)), nil
		case oid.T_timetz:
			if len(b) < 12 {
			...
			return ast.NewDTimeTZFromOffset(timeofday.TimeOfDay(timeOfDayMicros), offsetSecs), nil
		case oid.T_interval:
			if len(b) < 16 {
			...
			return &ast.DInterval{Duration: duration}, nil
		case oid.T_uuid:
			u, err := ast.ParseDUuidFromBytes(b)
			if err != nil {
				return nil, err
			}
			return u, nil
		case oid.T_inet:
			...
			return ast.NewDIPAddr(ast.DIPAddr{IPAddr: ipAddr}), nil
		case oid.T_jsonb:
			if len(b) < 1 {
		...
		case oid.T_varbit, oid.T_bit:
			...
			return &ast.DBitArray{BitArray: ba}, err
		default:
			if _, ok := types.ArrayOids[id]; ok {
	...
	case oid.T_bpchar:
		...
		return ast.NewDString(sv), nil
	case oid.T_name:
		if err := validateStringBytes(b); err != nil {
			return nil, err
		}
		return ast.NewDName(string(b)), nil
	default:
		return nil, errors.AssertionFailedf(
...
// it to PGEpochJDate.
func pgBinaryToDate(i int32) (*ast.DDate, error) {
	d, err := pgdate.MakeDateFromPGEpoch(i)
	if err != nil {
		return nil, err
	}
	return ast.NewDDate(d), nil
}
...
func decodeBinaryArray(
	ctx ast.ParseTimeContext, elemOid oid.Oid, b []byte, code FormatCode,
) (ast.Datum, error) {
	var hdr struct {
		Ndims int32
		// Nullflag
	...
	arr := ast.NewDArray(types.OidToType[elemOid])
	if hdr.Ndims == 0 {
		return arr, nil
	}
	...
	for i := int32(0); i < dim.DimSize; i++ {
		...
		if vlen < 0 {
			if err := arr.Append(ast.DNull); err != nil {
				return nil, err
			}
			continue
		}
