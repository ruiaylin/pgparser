...
// expected SQL string from the syntax ast. Note that if the input and output
...
$function$`,
	...
	for _, d := range testData {
		t.Run(d.sql, func(t *testing.T) {
			...
			s := stmts.StringWithFlags(ast.FmtShowPasswords)
			if d.expected != s {
...
// is properly reflected in the parse ast.
func TestParseTree(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.sql, func(t *testing.T) {
			...
			s := stmts.StringWithFlags(ast.FmtAlwaysGroupExprs)
			if d.expected != s {
				t.Errorf("%s: expected %s, but found (%d statements): %s", d.sql, d.expected, len(stmts), s)
			}
			if _, err := parser.Parse(s); err != nil {
...
func TestParsePrecedence(t *testing.T) {
	...
	one := ast.NewNumVal(constant.MakeInt64(1), "1", false /* negative */)
	minusone := ast.NewNumVal(constant.MakeInt64(1), "1", true /* negative */)
	two := ast.NewNumVal(constant.MakeInt64(2), "2", false /* negative */)
	minustwo := ast.NewNumVal(constant.MakeInt64(2), "2", true /* negative */)
	three := ast.NewNumVal(constant.MakeInt64(3), "3", false /* negative */)
	a := ast.NewStrVal("a")
	b := ast.NewStrVal("b")
	c := ast.NewStrVal("c")
