...
func prepareExpr(t *testing.T, datumExpr string) ast.Datum {
	...
	sema := ast.MakeSemaContext()
	typedExpr, err := ast.TypeCheck(ctx, expr, &sema, types.Any)
	if err != nil {
		t.Fatalf("%s: %v", datumExpr, err)
	}
...
func TestDFloatCompare(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	values := []ast.Datum{ast.DNull}
	for _, x := range []float64{math.NaN(), math.Inf(-1), -1, 0, 1, math.Inf(1)} {
		values = append(values, ast.NewDFloat(ast.DFloat(x)))
	}
...
// to ast.ParseDIntervalWithTypeMetadata beyond those in ast.ParseDInterval behave as expected.
func TestParseDIntervalWithTypeMetadata(t *testing.T) {
	...
	for _, td := range testData {
		actual, err := ast.ParseDIntervalWithTypeMetadata(td.str, td.dtype)
		if err != nil {
			t.Errorf("unexpected error while parsing INTERVAL %s %#v: %s", td.str, td.dtype, err)
			continue
		}
		expected, err := ast.ParseDInterval(td.expected)
		if err != nil {
...
func TestParseDDate(t *testing.T) {
	...
	for _, td := range testData {
		actual, depOnCtx, err := ast.ParseDDate(ctx, td.str)
		if err != nil {
			t.Errorf("unexpected error while parsing DATE %s: %s", td.str, err)
			continue
		}
		expected, _, err := ast.ParseDDate(nil, td.expected)
		if err != nil {
...
func TestParseDBool(t *testing.T) {
	...
	testData := []struct {
		...
		{str: "t", expected: ast.DBoolTrue},
		{str: "tr", expected: ast.DBoolTrue},
		{str: "tru", expected: ast.DBoolTrue},
		{str: "true", expected: ast.DBoolTrue},
		{str: "tr", expected: ast.DBoolTrue},
		{str: "TRUE", expected: ast.DBoolTrue},
		{str: "tRUe", expected: ast.DBoolTrue},
		{str: "  tRUe    ", expected: ast.DBoolTrue},
		{str: "  tR    ", expected: ast.DBoolTrue},
		{str: "on", expected: ast.DBoolTrue},
		{str: "On", expected: ast.DBoolTrue},
		{str: "oN", expected: ast.DBoolTrue},
		{str: "ON", expected: ast.DBoolTrue},
		{str: "1", expected: ast.DBoolTrue},
		{str: "yes", expected: ast.DBoolTrue},
		{str: "ye", expected: ast.DBoolTrue},
		{str: "y", expected: ast.DBoolTrue},

		{str: "false", expected: ast.DBoolFalse},
		{str: "FALSE", expected: ast.DBoolFalse},
		{str: "fALse", expected: ast.DBoolFalse},
		{str: "f", expected: ast.DBoolFalse},
		{str: "off", expected: ast.DBoolFalse},
		{str: "Off", expected: ast.DBoolFalse},
		{str: "oFF", expected: ast.DBoolFalse},
		{str: "OFF", expected: ast.DBoolFalse},
		{str: "0", expected: ast.DBoolFalse},
	...
	for _, td := range testData {
		t.Run(td.str, func(t *testing.T) {
			result, err := ast.ParseDBool(td.str)
			if td.err {
				if err == nil {
...
func TestParseDTime(t *testing.T) {
	...
	for _, td := range testData {
		actual, depOnCtx, err := ast.ParseDTime(ctx, td.str, td.precision)
		if err != nil {
			t.Errorf("unexpected error while parsing TIME %s: %s", td.str, err)
			continue
		}
		if *actual != ast.DTime(td.expected) {
...
func TestParseDTimeError(t *testing.T) {
	...
	for _, s := range testData {
		actual, _, _ := ast.ParseDTime(nil, s, time.Microsecond)
		if actual != nil {
...
func TestParseDTimeTZ(t *testing.T) {
	...
	for _, td := range testData {
		actual, depOnCtx, err := ast.ParseDTimeTZ(ctx, td.str, td.precision)
		if err != nil {
			t.Errorf("unexpected error while parsing TIME %s: %s", td.str, err)
			continue
		}
		exp := ast.DTimeTZ{TimeTZ: td.expected}
		if *actual != exp {
...
func TestParseDTimeTZError(t *testing.T) {
	...
	for _, s := range testData {
		actual, _, _ := ast.ParseDTimeTZ(nil, s, time.Microsecond)
		if actual != nil {
...
func TestParseDTimestamp(t *testing.T) {
	...
	for _, td := range testData {
		actual, depOnCtx, err := ast.ParseDTimestamp(ctx, td.str, time.Nanosecond)
		if err != nil {
...
func TestParseDTimestampTZ(t *testing.T) {
	...
	for _, td := range testData {
		actual, depOnCtx, err := ast.ParseDTimestampTZ(ctx, td.str, time.Nanosecond)
		if err != nil {
			t.Errorf("unexpected error while parsing TIMESTAMP %s: %s", td.str, err)
			continue
		}
...
func TestMakeDJSON(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	j1, err := ast.MakeDJSON(1)
	if err != nil {
		t.Fatal(err)
	}
	j2, err := ast.MakeDJSON(2)
	if err != nil {
		t.Fatal(err)
	}
	if j1.Compare(ast.NewTestingEvalContext(cluster.MakeTestingClusterSettings()), j2) != -1 {
...
func TestDTimeTZ(t *testing.T) {
	...
	maxTime, depOnCtx, err := ast.ParseDTimeTZ(ctx, "24:00:00-1559", time.Microsecond)
	require.NoError(t, err)
	require.False(t, depOnCtx)
	minTime, depOnCtx, err := ast.ParseDTimeTZ(ctx, "00:00:00+1559", time.Microsecond)
	require.NoError(t, err)
	require.False(t, depOnCtx)

	// These are all the same UTC time equivalents.
	utcTime, depOnCtx, err := ast.ParseDTimeTZ(ctx, "11:14:15+0", time.Microsecond)
	require.NoError(t, err)
	require.False(t, depOnCtx)
	sydneyTime, depOnCtx, err := ast.ParseDTimeTZ(ctx, "21:14:15+10", time.Microsecond)
	require.NoError(t, err)
	require.False(t, depOnCtx)
	...
	require.NoError(t, err)
	hawaiiTime := ast.NewDTimeTZFromLocation(timeofday.New(1, 14, 15, 0), hawaiiZone)

	weirdTimeZone := ast.NewDTimeTZFromOffset(timeofday.New(10, 0, 0, 0), -((5 * 60 * 60) + 30*60 + 15))

	testCases := []struct {
		t           *ast.DTimeTZ
		largerThan  []ast.Datum
		smallerThan []ast.Datum
		equalTo     []ast.Datum
		isMax       bool
		isMin       bool
	}{
		{
			t:           weirdTimeZone,
			largerThan:  []ast.Datum{minTime, ast.DNull},
			smallerThan: []ast.Datum{maxTime},
			equalTo:     []ast.Datum{weirdTimeZone},
			isMax:       false,
			isMin:       false,
		},
		{
			t:           utcTime,
			largerThan:  []ast.Datum{minTime, sydneyTime, ast.DNull},
			smallerThan: []ast.Datum{maxTime, hawaiiTime},
			equalTo:     []ast.Datum{utcTime},
			isMax:       false,
			isMin:       false,
		},
		{
			t:           sydneyTime,
			largerThan:  []ast.Datum{minTime, ast.DNull},
			smallerThan: []ast.Datum{maxTime, utcTime, hawaiiTime},
			equalTo:     []ast.Datum{sydneyTime},
			isMax:       false,
			isMin:       false,
		},
		{
			t:           hawaiiTime,
			largerThan:  []ast.Datum{minTime, utcTime, sydneyTime, ast.DNull},
			smallerThan: []ast.Datum{maxTime},
			equalTo:     []ast.Datum{hawaiiTime},
			isMax:       false,
			isMin:       false,
		},
		{
			t:           minTime,
			largerThan:  []ast.Datum{ast.DNull},
			smallerThan: []ast.Datum{maxTime, utcTime, sydneyTime, hawaiiTime},
			equalTo:     []ast.Datum{minTime},
			isMax:       false,
			isMin:       true,
		},
		{
			t:           maxTime,
			largerThan:  []ast.Datum{minTime, utcTime, sydneyTime, hawaiiTime, ast.DNull},
			smallerThan: []ast.Datum{},
			equalTo:     []ast.Datum{maxTime},
			isMax:       true,
			isMin:       false,
		},
	}
...
func TestIsDistinctFrom(t *testing.T) {
	...
	convert := func(s string) ast.Datums {
		splits := strings.Split(s, ",")
		result := make(ast.Datums, len(splits))
		for i, value := range splits {
			if value == "NULL" {
				result[i] = ast.DNull
				continue
			}
			result[i] = ast.NewDString(value)
		}
		return result
	}
	for _, td := range testData {
		t.Run(fmt.Sprintf("%s to %s", td.a, td.b), func(t *testing.T) {
			...
			if e, a := td.expected, datumsA.IsDistinctFrom(&ast.EvalContext{}, datumsB); e != a {
				if e {
...
func TestAllTypesAsJSON(t *testing.T) {
	...
	for _, typ := range types.Scalar {
		d := ast.SampleDatum(typ)
		_, err := ast.AsJSON(d, time.UTC)
		if err != nil {
...
var _ ast.ParseTimeContext = testParseTimeContext{}
