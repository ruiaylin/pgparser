...
func TestTypeCheck(t *testing.T) {
	...
	mapResolver := ast.MakeTestingMapTypeResolver(typeMap)
	...
	testData := []struct {
		...
		// type annotation is caused by the call to ast.Serialize, which formats the
	...
	for _, d := range testData {
		t.Run(d.expr, func(t *testing.T) {
			...
			if s := ast.Serialize(typeChecked); s != d.expected {
	...
	for _, d := range testData {
		t.Run(d.expr, func(t *testing.T) {
			...
			t.Run("semaCtx not nil", func(t *testing.T) {
				...
				if _, err := ast.TypeCheck(ctx, expr, &semaCtx, types.Any); !testutils.IsError(err, regexp.QuoteMeta(d.expected)) {
			...
			t.Run("semaCtx is nil", func(t *testing.T) {
				...
				if _, err := ast.TypeCheck(ctx, expr, nil, types.Any); !testutils.IsError(err, regexp.QuoteMeta(d.expected)) {
					t.Errorf("%s: expected %s, but found %v", d.expr, d.expected, err)
				}
			})
		})
