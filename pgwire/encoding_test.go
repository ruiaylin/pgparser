...
import (
	...
	"github.com/ruiaylin/pgparser/pkg/server/telemetry"
...
func readEncodingTests(t testing.TB) []*encodingTest {
	...
	for _, tc := range tests {
		...
		selectStmt, ok := stmt.AST.(*ast.Select)
		if !ok {
			t.Fatal("not select")
		}
		selectClause, ok := selectStmt.Select.(*ast.SelectClause)
		if !ok {
...
func TestExoticNumericEncodings(t *testing.T) {
	...
	for i, c := range testCases {
		t.Run(fmt.Sprintf("%d_%s", i, c.Value), func(t *testing.T) {
			...
			expected := &ast.DDecimal{Decimal: *c.Value}
