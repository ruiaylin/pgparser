...
import (
	...
	"github.com/ruiaylin/pgparser/utils/metric"
	"github.com/ruiaylin/pgparser/utils/timeutil"
	"github.com/ruiaylin/pgparser/utils/uuid"
...
func TestParseTs(t *testing.T) {
	...
	for i, test := range parseTsTests {
		parsed, _, err := ast.ParseDTimestamp(nil, test.strTimestamp, time.Nanosecond)
		if err != nil {
...
func TestTimestampRoundtrip(t *testing.T) {
	...
	parse := func(encoded []byte) time.Time {
		decoded, _, err := ast.ParseDTimestamp(nil, string(encoded), time.Nanosecond)
		if err != nil {
			t.Fatal(err)
		}
		return decoded.UTC()
...
func TestWriteBinaryArray(t *testing.T) {
	...
	ary, _, _ := ast.ParseDArrayFromString(ast.NewTestingEvalContext(st), "{1}", types.Int)
...
func TestIntArrayRoundTrip(t *testing.T) {
	...
	d := ast.NewDArray(types.Int)
	for i := 0; i < 10; i++ {
		if err := d.Append(ast.NewDInt(ast.DInt(i))); err != nil {
			t.Fatal(err)
		}
	}
...
func TestFloatConversion(t *testing.T) {
	...
	for _, test := range testData {
		t.Run(fmt.Sprintf("%g/%d", test.val, test.extraFloatDigits), func(t *testing.T) {
			...
			d := ast.NewDFloat(ast.DFloat(test.val))
			buf.writeTextDatum(context.Background(), d, defaultConv, nil /* t */)
			b := buf.wrapped.Bytes()
...
func TestByteArrayRoundTrip(t *testing.T) {
	...
	randValues := make(ast.Datums, 0, 11)
	randValues = append(randValues, ast.NewDBytes(ast.DBytes("\x00abc\\\n")))
	for i := 0; i < 10; i++ {
...
func benchmarkWriteType(b *testing.B, d ast.Datum, format pgwirebase.FormatCode) {
	ctx := context.Background()

	buf := newWriteBuffer(nil /* bytecount */)
	buf.bytecount = metric.NewCounter(metric.Metadata{Name: ""})

	writeMethod := func(ctx context.Context, d ast.Datum, loc *time.Location) {
		defaultConv := makeTestingConvCfg()
...
func benchmarkWriteTimestamp(b *testing.B, format pgwirebase.FormatCode) {
	ts, _, err := ast.ParseDTimestamp(nil, "2010-09-28 12:00:00.1", time.Microsecond)
	if err != nil {
		b.Fatal(err)
	}
...
func benchmarkWriteTimestampTZ(b *testing.B, format pgwirebase.FormatCode) {
	tstz, _, err := ast.ParseDTimestampTZ(nil, "2010-09-28 12:00:00.1", time.Microsecond)
	if err != nil {
		b.Fatal(err)
	}
...
func benchmarkWriteInterval(b *testing.B, format pgwirebase.FormatCode) {
	i, err := ast.ParseDInterval("PT12H2M")
	if err != nil {
		b.Fatal(err)
	}
